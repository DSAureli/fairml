...
def verify_black_box_function(predict_method, number_of_features,
	...
	covariance = np.eye(number_of_features)	# np.eye(N) -> NxN array with ones on the diagonal and zeros elsewhere
	mean = np.zeros(number_of_features)	# number_of_features-dimensional array filled with zeros

	data = np.random.multivariate_normal(mean, covariance, number_of_data_points)
	try:
		output = predict_method(data)

		# check to make sure that the estimator returns a numpy array
